...
<body>
    ...
    <div class="container d-flex justify-content-center p-5">
        <div class="row text-center p-5">
            ...
            <div class="w-100 p-3">
                <a class="btn btn-primary" href="{{ route('vaccine.list') }}" role=" button">Register Patient</a>
            </div>
        </div>
    </div>
    <!---->
    ...
    <div class="container p-2">
        <h3 class="text-center p-5"> List Patient</h3>
        <a class="btn btn-primary" href="{{ route('vaccine.list') }}" role="button">Register Patient</a>
        <div class="py-3">
                ...
                <tbody>
                    ...
                    <tr>
                        <th scope="row"><?php echo $x;
                                        $x++ ?></th>
                        <td> {{$patients->vaccine->name}}</td>
                        <td>{{$patients->name}}</td>
                        <td> {{$patients->nik}}</td>
                        <td> {{$patients->alamat}}</td>
                        <td>{{$patients->no_hp}}</td>
                        <td><a type="button" href="{{ route('patient.edit',$patients->id) }}" class="btn btn-warning">Edit</a>
                            <form action="{{ route('patient.destroy',$patients->id) }}" method="post" class="d-inline">
                                @csrf
                                @method('delete')
